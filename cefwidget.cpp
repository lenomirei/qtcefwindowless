...
void CefWidget::OnPaint(CefRefPtr<CefBrowser> browser,
  ...
  if (!frame_buffer_ || width != width_ || height != height_) {
    ...
    if (!timer_->isActive()) {
      QMetaObject::invokeMethod(this, [this]() { timer_->start(); });
    }
  }
...
void CefWidget::initializeGL() {
  ...
  shader_.setAttributeBuffer(texLoc, GL_FLOAT, 2 * sizeof(GLfloat), 2,
                             4 * sizeof(GLfloat));
...
void CefWidget::UpdateFrame(const uchar* buffer, int width, int height) {
  ...
  texture_->setData(QOpenGLTexture::QOpenGLTexture::BGRA, QOpenGLTexture::UInt8,
                    buffer);
